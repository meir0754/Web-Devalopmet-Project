...
function SubmitContactForm(i_this, i_event){
    ...
    myCaller.sendCustomerApply(o_Requset, myPresentor.GetCurrMsgBox(), true, function(i_response){
        if (i_response.valid) myPresentor.RespondValidMsg('פנייתך התקבלה בהצלחה. נציג יצור איתך קשר בהקדם.'); 
        else if (i_response.valid == undefined || i_response.valid == null) myPresentor.RespondInternalError(i_response);
        else if (!i_response.valid && i_response.msg.indexOf('mail') != -1) myPresentor.RespondErrorMsg('כתובת המייל אינה תקינה');
        else if (!i_response.valid && i_response.msg.indexOf('phone') != -1) myPresentor.RespondErrorMsg('מספר הטלפון אינו תקין');
